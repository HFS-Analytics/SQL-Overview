...
*Note: Use single quotes `'` to mark text strings. Double quotes (`"`) are an alternative way to indicate a column name and can lead to unexpected results.*
...
The `WHERE` clause filters the result set we create in the `FROM` clause down to the rows we want to keep. The `WHERE` clause amounts to a long logical test applied to each row. If the result is true then the row is kept; otherwise it is removed from the result set.
...
Now hold on a minute! Rosie the mutt made it onto this list, even though we specifically asked for mixed breeds to be excluded with `Breeds.Breed_Name <> 'Mixed'`! What happened?
